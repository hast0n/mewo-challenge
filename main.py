...
# ------------------------------------------------------------------------
param_string = "[LAUNCH] Using %s parameters (mode: %i, metric: \"%s\", precision: %i)"
...
if (len(sys.argv) > 1):
    ...
    try: sys.argv[3] if sys.argv[3] in _METRICS_.keys() else _DEFAULT_METRIC_
    except (IndexError): print(warn_string % ("\nmetric\n", _DEFAULT_METRIC_))

print(param_string % (param, mode, precision, metric))
# ------------------------------------------------------------------------
DRIVE_FOLDER_PATH = './'
